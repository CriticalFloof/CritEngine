...
namespace Serialization {
	...
	void ToJsonInner(T&& in, std::string& out)
	{
		...
		*/
		...
		using StrippedType = typename std::remove_cvref_t<T>;
		...
		else if constexpr (refl::trait::is_reflectable_v<StrippedType>)
		{
			...
			refl::util::for_each(refl::reflect(in).members, [&]<typename MD>(const MD member, size_t memberIndex)
			{
	...
	void FromJson(std::string_view& in, T& out)
	{
		...
		else if constexpr (refl::trait::is_reflectable_v<StrippedType>)
		{
			...
			refl::util::for_each(refl::reflect(out).members, [&]<typename MD>(const MD member, size_t memberIndex)
			{
				if constexpr (refl::trait::is_field_v<MD>)
				{
					using MT = MD::value_type;
