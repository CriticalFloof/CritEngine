...
namespace Engine
{
    ...
    ) : m_rotation(rotation), m_position(position), m_aspectRatio(aspect_ratio), m_nearPlane(near_plane),
    ...
    void PerspectiveCamera::calculateViewMatrix()
    {
        ...
        m_viewMatrix = Matrix4f({
            ...
            // 4th row
            -p.x * (1 - 2 * q.y * q.y - 2 * q.z * q.z) - p.y * (2 * q.x * q.y - 2 * q.w * q.z) - p.z * (2 * q.x * q.z +
                2 * q.w * q.y),
            -p.x * (2 * q.x * q.y + 2 * q.w * q.z) - p.y * (1 - 2 * q.x * q.x - 2 * q.z * q.z) - p.z * (2 * q.y * q.z -
                2 * q.w * q.x),
            -p.x * (2 * q.x * q.z - 2 * q.w * q.y) - p.y * (2 * q.y * q.z + 2 * q.w * q.x) - p.z * (1 - 2 * q.x * q.x -
                2 * q.y * q.y),
            1
        });
